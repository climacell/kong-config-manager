      ...
      .tap(remoteObjs => {
        ...
        return sequence([
          ...
          sequence(newItemIds, newItemId => {
            ...
            switch (obj) {
              case 'plugins':
                newItemUrl = newItemObj.api_id
                  ? `${host}/apis/${newItemObj.api_id}/plugins/`
                  : `${host}/plugins`;
                break;
              case 'jwts':
                newItemUrl = newItemObj.consumer_id
                  ? `${host}/consumers/${newItemObj.consumer_id}/jwt/`
                  : undefined;
                break;
              case 'key-auths':
                newItemUrl = newItemObj.consumer_id
                  ? `${host}/consumers/${newItemObj.consumer_id}/key-auth/`
                  : undefined;
                break;
              case 'acls':
                newItemUrl = newItemObj.consumer_id
                  ? `${host}/consumers/${newItemObj.consumer_id}/acls/`
                  : undefined;
                break;
              default:
                newItemUrl = isTarget
