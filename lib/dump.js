...
module.exports = function dump(config, name) {
  ...
  return getObjects(host).then(OBJECTS => {
    ...
    if (config.object_type) {
      objects = objects.filter(obj => obj === config.object_type);
    }

    const removeRootDir = config.object_type === undefined;

    debug(`Should remove root directory: ${removeRootDir}`);
    logger.info(`Ready to dump configs of kong instance ${name}...`);
    makeDir(filenameConverter.serialize(name), removeRootDir);
